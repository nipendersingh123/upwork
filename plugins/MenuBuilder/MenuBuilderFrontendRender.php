...
use plugins\MenuBuilder\MenuBuilderSetup;
use App\Models\Menu;
...
class MenuBuilderFrontendRender
{
    ...
    {
//        $attributes_string = property_exists($menu_item,'children') && ! empty($menu_item->children) ? ['class' => ['menu-item-has-children']]  : [];

        $attributes_string = (property_exists($menu_item, 'children') && is_array($menu_item->children) && !empty(array_filter($menu_item->children, function($child) {
                return !empty((array) $child); // Convert to array to check if child has any data
            })))
